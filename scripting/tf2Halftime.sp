...
public void OnMapStart()
{
	...
	else 												// or something else.
	{
		half1Limit = -1;
		totalWinLimit = -1;
		CPrintToChatAll("{mediumpurple}[tf2Halftime] {yellow}Warning!{white} tf2Halftime is somehow running on an unsupported map. Expect issues!"); // plugin should not be running here but if it is say something in chat about it
	}
}
...
{
	if (tourneyRestart) // if a winlimit has been reached, restart the tournament mode on the next round start (instead of immediately) to prevent accidental shortening of logs
	{
